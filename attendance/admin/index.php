...
if(isset($_POST['submit'])) {
  ...
  if($email != "" && $password != "") {
    try {
      $query = "select id, name, email, mobile, password from admin where email=:email and password=:password";
...
$newpassword=md5($_POST['newpwd']);
  $sql ="SELECT email FROM admin WHERE email=:email and mobile=:mobile";
...
if($query -> rowCount() > 0)
{
$con="update admin set password=:newpassword where email=:email and mobile=:mobile";
