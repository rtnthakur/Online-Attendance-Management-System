...
if(isset($_POST['submit'])) {
  ...
  if($email != "" && $password != "") {
    try {
      $query = "select id, EmpId, fname, lname, department_name, email, mobile, country, state, city, address, photo, dob, date_of_joining, password from employee where email=:email and password=:password";
...
$newpassword=md5($_POST['newpwd']);
  $sql ="SELECT EmpId FROM employee WHERE EmpId=:empid and mobile=:mobile";
...
if($query -> rowCount() > 0)
{
$con="update employee set password=:newpassword where EmpId=:empid and mobile=:mobile";
...
<html>
  <head>
    ...
    <title>Login - Staff Attendance System Admin</title>
  </head>
  <body>
